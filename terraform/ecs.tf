...
resource "aws_iam_role_policy" "ecs_task_secrets_access" {
  name = "${var.project_name}-ecs-task-secrets-access"
  role = aws_iam_role.ecs_task_role.id
  ...
  container_definitions = jsonencode([
    {
      ...
      secrets = [
        ...
        {
          name      = "EMAIL_HOST"
          valueFrom = "${aws_secretsmanager_secret.app_secrets.arn}:EMAIL_HOST::"
        },
        {
          name      = "EMAIL_PORT"
          valueFrom = "${aws_secretsmanager_secret.app_secrets.arn}:EMAIL_PORT::"
        },
        {
          name      = "EMAIL_USER"
        ...
        {
          name      = "EMAIL_PASSWORD"
          valueFrom = "${aws_secretsmanager_secret.app_secrets.arn}:EMAIL_PASSWORD::"
        },
        {
          name      = "EMAIL_FROM"
      ...
      healthCheck = {
        command     = ["CMD-SHELL", "wget --no-verbose --tries=1 --spider http://localhost:${var.backend_port}/health || exit 1"]
        interval    = 30
        timeout     = 5
        retries     = 3
        startPeriod = 60
    ...
    },
    {
      ...
      healthCheck = {
        command     = ["CMD-SHELL", "wget --no-verbose --tries=1 --spider http://localhost:${var.frontend_port}/ || exit 1"]
        interval    = 30
        timeout     = 5
        retries     = 3
        startPeriod = 30
